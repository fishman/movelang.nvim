...
Please refer to the move-analyzer documentation at
https://move-analyzer.github.io/manual.html#%s
...
---@param tbl table
---@param json_key string e.g. "move-analyzer.check.overrideCommand"
...
---@param tbl table
---@param json_tbl { [string]: unknown }
function M.override_with_move_analyzer_json_keys(tbl, json_tbl)
  M.override_with_json_keys(tbl, json_tbl, function(key)
    return vim.startswith(key, 'move-analyzer')
  end)
end
