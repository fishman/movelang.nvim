...
---@param health movelang.lsp_server_health_status
---@return boolean
...
---@param result movelang.internal.RAInitializedStatus
...
function M.handler(_, result, ctx, _)
  ...
  if is_notify_enabled_for(result.health) then
    local message = ([[
move-analyzer health status is [%s]:
%s
Run ':RustLsp logFile' for details.
To configure or disable move-analyzer server status notifications,
see ':h movelang.lsp.ClientOpts'.
  ...
  -- move-analyzer may provide incomplete/empty inlay hints by the time Neovim
  ...
  -- as soon as move-analyzer has fully initialized.
