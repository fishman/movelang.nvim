...
---@class movelang.hover_actions.State
local _state = {
  ---@type integer
  winnr = nil,
  ---@type unknown
...
local function execute_move_analyzer_command(action, ctx)
...
local function run_command(ctx, line)
  ...
  close_hover()
  execute_move_analyzer_command(action, ctx)
end
...
local function parse_commands()
  ...
  for i, value in ipairs(_state.commands) do
    if value.command == 'move-analyzer.gotoLocation' then
      table.insert(prompt, string.format('%d. Go to %s (%s)', i, value.title, value.tooltip))
    elseif value.command == 'move-analyzer.showReferences' then
...
function M.handler(_, result, ctx)
  ...
  local bufnr, winnr = lsp_util.open_floating_preview(
    ...
    vim.tbl_extend('keep', win_opt, {
      focusable = true,
      focus_id = 'move-analyzer-hover-actions',
  ...
  vim.keymap.set('n', '<Plug>MoveHoverAction', function()
...
--- Sends the request to move-analyzer to get hover actions and handle it
