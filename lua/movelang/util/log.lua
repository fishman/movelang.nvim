...
function log.notify(scope, level, verbose, str, ...)
  if not verbose and _G.Movelang.config ~= nil and not _G.Movelang.config.debug then
    return
  end
  ...
  vim.notify(string.format('[movelang.nvim@%s] %s', scope, string.format(str, ...)), level, { title = 'movelang.nvim' })
end
...
function log.warn_deprecation(options)
  ...
  if uses_deprecated_option then
    ...
    log.notify('deprecated_options', vim.log.levels.WARN, true, 'use `:h Movelang.options` to read more.')
  end
end
