---@mod movelang.neotest
---
---@brief [[
---
---A |neotest| adapter for move, powered by movelang.
---
...
---    adapters = {
---      -- ...,
---      require('movelang.neotest')
---    },
---}
---<
---
---this plugin will configure itself to use |neotest|
---as a test executor, and |neotest| will use move-analyzer
---for test discovery and command construction.
---
---Note: If you use this adapter, do not add the neotest-move adapter
---(another plugin).
---
---@brief ]]
...
local cargo = require('movelang.cargo')
...
local NeotestAdapter = { name = 'movelang' }
...
---@package
---@class movelang.neotest.Position: neotest.Position
...
---@package
---@class nio.movelang.Client: nio.lsp.Client
---@field request nio.movelang.RequestClient Interface to all requests that can be sent by the client
---@field config vim.lsp.ClientConfig

---@package
---@class nio.movelang.RequestClient: nio.lsp.RequestClient
---@field experimental_runnables fun(args: nio.lsp.types.ImplementationParams, bufnr: integer?, opts: nio.lsp.RequestOpts): nio.lsp.types.ResponseError|nil, movelang.RARunnable[]|nil

---@package
---@param file_path string
---@return neotest.Tree
NeotestAdapter.discover_positions = function(file_path)
  ---@type movelang.neotest.Position[]
  local positions = {}

  local lsp_client = require('movelang.move_analyzer').get_client_for_file(file_path, 'experimental/runnables')
  if not lsp_client then
    ---@diagnostic disable-next-line: missing-parameter
    return lib.positions.parse_tree(positions)
  end
  local nio_client = nio.lsp.get_client_by_id(lsp_client.id)
  ---@cast nio_client nio.movelang.Client
  ...
  ---@cast runnables movelang.RARunnable[]
  ...
  ---@type { [string]: movelang.neotest.Position[] }
...
---@package
---@class movelang.neotest.RunSpec: neotest.RunSpec
---@field context movelang.neotest.RunContext

---@package
---@class movelang.neotest.RunContext
---@field file string
---@field pos_id string
...
function NeotestAdapter.build_spec(run_args)
  ...
  local pos = tree:data()
  ---@cast pos movelang.neotest.Position
  ...
  local exe, args, cwd = require('movelang.runnables').get_command(runnable)
  if run_args.strategy == 'dap' then
    local dap = require('movelang.dap')
    ...
    ---@cast strategy movelang.dap.client.Config
    ---@type movelang.neotest.RunSpec
    local run_spec = {
      cwd = cwd,
      context = context,
      strategy = strategy,
...
function NeotestAdapter.results(spec, strategy_result)
  ...
  local results = {}
  ---@type movelang.neotest.RunContext
  ...
  local diagnostics = require('movelang.test').parse_diagnostics(context.file, output_content)
  ...
  if has_failures then
    require('movelang.neotest.parser').populate_pass_positions(results, context, output_content)
  end
  return results
end
