...
local move_analyzer = require('movelang.move_analyzer')
local server_status = require('movelang.server_status')
...
---Searches upward for a .vscode/settings.json that contains move-analyzer
...
local function find_vscode_settings(bufname)
  ...
  return content and require('movelang.config.json').silent_decode(content) or {}
end
...
local function get_start_settings(bufname, root_dir, client_config)
  ...
  if config.server.load_vscode_settings then
    ...
    require('movelang.config.json').override_with_move_analyzer_json_keys(evaluated_settings, json_settings)
  end
...
--- configures move-analyzer to enable server-side file watching (if not configured otherwise).
---
---@param server_cfg movelang.lsp.StartConfig LSP start settings
local function configure_file_watcher(server_cfg)
  ...
  local file_watcher_setting = vim.tbl_get(server_cfg.settings, 'move-analyzer', 'files', 'watcher')
...
local function restart(bufnr, filter, callback)
  ...
  timer:start(200, 100, function()
    ...
    elseif attempts_to_live <= 0 then
      vim.notify('movelang.lsp: Could not restart all LSP clients.', vim.log.levels.ERROR)
      timer:stop()
      attempts_to_live = 0
    end
...
---@class movelang.lsp.StartConfig: movelang.lsp.ClientConfig
...
M.start = function(bufnr)
  ...
  ---@type movelang.lsp.StartConfig
  local lsp_start_config = vim.tbl_deep_extend('force', {}, client_config)
  local root_dir = move.get_root_dir(bufname)
  ...
  -- move-analyzer treats settings in initializationOptions specially -- in particular, workspace_discoverConfig
  -- so copy them to init_options (the vim name)
  -- so they end up in initializationOptions (the LSP name)
  -- ... and initialization_options (the rust name) in move-analyzer's main.rs
  lsp_start_config.init_options = vim.tbl_deep_extend(
    'force',
    lsp_start_config.init_options or {},
    vim.tbl_get(lsp_start_config.settings, 'move-analyzer')
  )

  -- Check if a client is already running and add the workspace folder if necessary.
  for _, client in pairs(move_analyzer.get_active_movelang_clients()) do
  ...
  local move_analyzer_cmd = types.evaluate(client_config.cmd)
  -- special case: move-analyzer has a `move-analyzer.server.path` config option
  -- that allows you to override the path via .vscode/settings.json
  local server_path = vim.tbl_get(lsp_start_config.settings, 'move-analyzer', 'server', 'path')
  if type(server_path) == 'string' then
    move_analyzer_cmd[1] = server_path
    --
  end
  if #move_analyzer_cmd == 0 then
    vim.notify('move-analyzer command is not set!', vim.log.levels.ERROR)
    return
  end
  if vim.fn.executable(move_analyzer_cmd[1]) ~= 1 then
    vim.notify(('%s is not executable'):format(move_analyzer_cmd[1]), vim.log.levels.ERROR)
    return
  end
  ---@cast move_analyzer_cmd string[]
  lsp_start_config.cmd = move_analyzer_cmd
  lsp_start_config.name = 'move-analyzer'
  ...
  if config.tools.hover_actions.replace_builtin_hover then
    custom_handlers['textDocument/hover'] = require('movelang.hover_actions').handler
  end
  ...
  lsp_start_config.on_init = function(...)
    ...
    commands.create_move_lsp_command()
  ...
  lsp_start_config.on_attach = function(...)
    ...
    if config.dap.autoload_configurations then
      ...
      require('movelang.commands.debuggables').add_dap_debuggables()
    end
  end
  ...
  lsp_start_config.on_exit = function(...)
    ...
    vim.schedule(function()
      commands.delete_move_lsp_command()
    end)
...
M.stop = function(bufnr, filter)
  bufnr = bufnr or vim.api.nvim_get_current_buf()
  local clients = move_analyzer.get_active_movelang_clients(bufnr, filter)
...
M.reload_settings = function(bufnr)
  bufnr = bufnr or vim.api.nvim_get_current_buf()
  local clients = move_analyzer.get_active_movelang_clients(bufnr)
  ---@cast clients vim.lsp.Client[]
  for _, client in ipairs(clients) do
...
---@enum MoveAnalyzerCmd
local MoveAnalyzerCmd = {
  start = 'start',
  stop = 'stop',
  restart = 'restart',
...
local function move_analyzer_cmd(opts)
  local fargs = opts.fargs
  local cmd = fargs[1]
  ---@cast cmd MoveAnalyzerCmd
  if cmd == MoveAnalyzerCmd.start then
    M.start()
  elseif cmd == MoveAnalyzerCmd.stop then
    M.stop()
  elseif cmd == MoveAnalyzerCmd.restart then
    M.restart()
  elseif cmd == MoveAnalyzerCmd.reload_settings then
    M.reload_settings()
  end
end

vim.api.nvim_create_user_command('MoveAnalyzer', move_analyzer_cmd, {
  nargs = '+',
  desc = 'Starts, stops the move-analyzer LSP client or changes the target',
  complete = function(arg_lead, cmdline, _)
    local clients = move_analyzer.get_active_movelang_clients()
    ---@type MoveAnalyzerCmd[]
    local commands = #clients == 0 and { 'start' } or { 'stop', 'restart', 'reloadSettings' }
    if cmdline:match('^MoveAnalyzer%s+%w*$') then
