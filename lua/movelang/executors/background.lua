local diag_namespace = vim.api.nvim_create_namespace('movelang')
...
M.execute_command = function(command, args, cwd, opts)
  ...
  vim.system(cmd, { cwd = cwd }, function(sc)
    ...
    local diagnostics = require('movelang.test').parse_diagnostics(fname, output)
    local summary = get_test_summary(sc.stdout or '')
    vim.schedule(function()
